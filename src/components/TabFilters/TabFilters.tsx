...
const TabFilters = ({ sortItem, sortItems, setSortItem }) => {
    ...
    const renderTabsSortOrder = () => {
        const sortTitle =
            (sortItem &&
                sortItems.filter((i) => i.id === sortItem)?.[0]?.name) ||
            "Sort order";
        return (
            <Popover className="relative">
                {({ open, close }) => (
              ...
              ${open ? "!border-primary-500 " : ""}
                ...
                `}
                        >
                            ...
                            <span className="ml-2">{sortTitle}</span>
                            {!sortItem.length ? (
                                <ChevronDownIcon className="w-4 h-4 ml-3" />
                            ) : (
                                <span onClick={() => setSortItem("")}>
                                    {renderXClear()}
                                </span>
                            )}
                        </Popover.Button>
                                        ...
                                        {DATA_sizes.map((item) => (
                                            <div key={item.name} className="">
                                                <Checkbox
                                                    name={item.name}
                                                    label={item.name}
                                                    defaultChecked={sizesState.includes(
                                                        item.name
                                                    )}
                                                    onChange={(checked) =>
                                                        handleChangeSizes(
                    ...
                    <>
                        ...
                        >
                            ...
                            rangePrices[1] === PRICE_RANGE[1] ? null : (
                                <span
                                    onClick={() => setRangePrices(PRICE_RANGE)}
                                >
                                    {renderXClear()}
                                </span>
                            )}
